...
func newMatchEntity(data matchSchema) *domain.Match {
...
func (repo *MatchRepository) Find(id string) *domain.Match {
	...
	data := raw.(matchSchema)

	match := newMatchEntity(data)
	return match
}
...
func (repo *MatchRepository) Save(match *domain.Match) error {
	...
	return matches.Update(match.ID, matchSchema{
		ID:          match.ID,
		State:       match.State(),
		...
		Teamd2Ready: match.Team2().IsReady,
	})
}
